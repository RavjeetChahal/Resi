...
const HomeScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    if (!user?.uid) return;
    console.log("[HomeScreen] Setting up Firebase listener for tickets");
    setLoadingTickets(true);
    try {
      const db = getFirebaseDatabase();
      if (!db) {
        console.warn("[HomeScreen] Firebase database not available");
        return;
      }
      const ticketsRef = ref(db, "tickets");
      const unsubscribe = onValue(ticketsRef, (snapshot) => {
        console.log(
          "[HomeScreen] Firebase snapshot received, updating tickets"
        );
        ...
        const normalizeTicket = ([id, ticket]) => {
          ...
          if (!team) {
            // Infer team from category if not explicitly set
            team = category === "Resident Life" ? "ra" : "maintenance";
          }
          ...
          return {
            ...
            category,
            issueType: ticket.issue_type || ticket.issueType || "General",
            team,
            raw: ticket,
          };
        };
  ...
  const startRecordingAsync = useCallback(async () => {
    ...
    try {
      ...
      log(
        "Native recording prepared. Status",
        ...
        {loadingTickets ? (
          ...
          <>
            <FlatList
              ...
              renderItem={({ item }) => {
                // Debug logging
                console.log("[HomeScreen] Ticket data:", {
                  id: item.id,
